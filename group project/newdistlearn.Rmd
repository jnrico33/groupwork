...
##Is this neccessary if the ggplot will already omit NA's? Keeping our code shorter but more concise seems preferable.
```{r}
gpa2 %>% 
  group_by(year) %>% 
  summarise_all(~(sum(is.na(.))))
...
```
#Attempt to merge all attendance datasets (Do you plan to use 'virtualed' in the apps?)
